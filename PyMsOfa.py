...
#gcc -shared -fPIC -o libsofa_c.so *.c  #creat sofa_c shared libary
#path = os.getcwd()
#lib  = CDLL(os.path.join(path, 'libsofa_c.so'))

#lib = CDLL( CDLL'./libsofa_c.so')
#lib = CDLL('E:\Python\p2c\libsofa_c.so')


#ref. from pysofa_ctypes
#for windows
...
#def req_shape_c(a, dtype=None, shape=None, req=None):
#    return np.require(a, dtype=dtype, requirements=req).reshape(shape, order='C')
...
def pymA2af(ndp, angle):
    ...
    #lib.iauA2af.argtypes = [c_int, c_double, POINTER(c_char), c_int * 4]
...
def pymS2c_A(theta, phi, c):
    ...
    #array_1d_double = nt.ndpointer(shape=(1,3), dtype=np.double, flags='C')  
    #lib.iauS2c.argtypes = [c_double, c_double, array_1d_double]
...
def pymTaitt(tai1, tai2):
    ...
    '''
    #lib.iauTaitt.argtypes = [c_double, c_double, POINTER(c_double), POINTER(c_double)]
...
class iauASTROM(ctypes.Structure):
    ...
    def print(iauASTROM):
        ...
        eb_c = tuple([x for x in iauASTROM.eb])
        eh_c = tuple([x for x in iauASTROM.eh])
        v_c  = tuple([x for x in iauASTROM.v])
        #v_c  = np.array([x for x in iauASTROM.v])
        #bpn_c= tuple([x for x in iauASTROM.bpn])
        bpn_c  = np.array([x for x in iauASTROM.bpn])
        print("pmt:",    iauASTROM.pmt)
        print("eb:",     eb_c)
        print("eh:",     eh_c)
        print("em:",     iauASTROM.em) 
        print("v:",      v_c)
        print("bm1:",    iauASTROM.bm1) 
        print("bpn:",    bpn_c)
        print("along:",  iauASTROM.along)   
        print("phi:",    iauASTROM.phi)     
        print("xpl:",    iauASTROM.xpl)    
        print("ypl:",    iauASTROM.ypl)    
        print("sphi:",   iauASTROM.sphi)    
        print("cphi:",   iauASTROM.cphi)    
        print("diurab:", iauASTROM.diurab) 
        print("eral:",   iauASTROM.eral)
        print("refa:",   iauASTROM.refa)  
        print("refb:",   iauASTROM.refb)
...
class iauASTROM(ctypes.Structure):
    ...
    def __init__(self, iauASTROM):
    
        self.pmt   = iauASTROM.pmt      
        self.eb    = iauASTROM.eb       
        self.eh    = iauASTROM.eh       
        self.em    = iauASTROM.em       
        self.v     = iauASTROM.v        
        self.bm1   = iauASTROM.bm1      
        self.bpn   = iauASTROM.bpn      
        self.along = iauASTROM.along    
        self.phi   = iauASTROM.phi      
        self.xpl   = iauASTROM.xpl      
        self.ypl   = iauASTROM.ypl      
        self.sphi  = iauASTROM.sphi     
        self.cphi  = iauASTROM.cphi     
        self.diurab= iauASTROM.diurab   
        self.eral  = iauASTROM.eral     
        self.refa  = iauASTROM.refa     
        self.refb  = iauASTROM.refb     
...
#Note: no pointer here, use byref to return structure/class    
    astrom  = iauASTROM()
    #lib.iauApcg(date1, date2, ebpv,  ehp,  byref(astrom))
...
''' 
c_body_p =  POINTER(iauLDBODY)
#c_body_3 =  iauLDBODY*3
#it should debug for struct iauLDBODY       
def pymAtciqn_A(rc,  dc,  pr,  pd,  px,  rv,  astrom,  n,  b):
...
def pymTporv(xi,   eta,   v):
    ...
    #if   j == 0:
    #     raise ValueError(iau_tporv_msg[j])
    #elif j > 0:
    #     ws.warn(iau_tporv_msg[j], UserWarning, 2)    
...
#iau_CR: to be done in vector tools

#iau_DAT: has been done in time tools
...
def  iauPrec76(date01, date02, date11, date12):
    '''
...
#2023-05-27 
#'''
#iau_RX, RXP, RXR, RY, RZ, TR to be done in VM
#'''   
...
def pymAf2a(s,   ideg,   iamin,   asec):
    ...
    if j > 0:
        ...
        #raise ValueError(iau_af2a_msg[j])
